...
namespace CryptoPortfolioTracker.Services
{
    public class TransactionService : ITransactionService
    {
                ...
                switch (mutation.Direction.ToString())
                {
                    case "In":
                        if ((asset.Qty + mutation.Qty) != 0) //** prevent divide by zero
                        {
                            ...
                            asset.Qty = asset.Qty + mutation.Qty;
                        }
                        else
                        {
                ...
                switch (mutationNew.Direction.ToString())
                {
                    case "In":
                        ...
                        if (asset.Qty - mutationToEdit.Qty != 0)
                        {
                            ...
                            asset.Qty = asset.Qty - mutationToEdit.Qty;
                        }
                        else
                        {
                            asset.AverageCostPrice = 0;
        ...
        private async Task<Result<Asset>> ReverseAndRecalculateFee(Mutation mutationNew, Mutation mutationToEdit)
        {
            ...
            try
            {
                ...
                if (mutationToEdit.Asset.Id != mutationNew.Asset.Id || mutationToEdit.Qty != mutationNew.Qty)
                {
                    ...
                    assetOld.Qty = assetOld.Qty + mutationToEdit.Qty;
                    ...
                    assetNew.Qty = assetNew.Qty - mutationNew.Qty;
                }
            }
            catch (Exception ex)
            {
        ...
        //public async Task<bool> EditTransactionNew(AssetTransaction transactionNew, AssetTransaction transactionToEdit)
        //{
        ...
        public async Task<Result<bool>> AddTransaction(AssetTransaction transaction)
        {
            ...
            try
            {
                foreach (var mutation in mutations)
                {
                    ...
                    {
                        mutation.Asset = (await CreateNewAsset(mutation))
                           .Match(Succ: asset => asset, Fail: err => throw new Exception(err.Message, err));
        ...
        {
            var dummyMutation = new Mutation();
        ...
        private object CloneMutation(Mutation mut)
        {
            ...
            foreach (var property in properties)
            {
                ...
                cloneMut.GetType().GetProperty(property.Name).SetValue(cloneMut, value);
            }
            return cloneMut;
        }
